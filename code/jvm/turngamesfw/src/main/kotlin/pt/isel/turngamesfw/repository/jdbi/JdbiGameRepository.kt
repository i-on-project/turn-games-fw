...
class JdbiGameRepository(private val handle: Handle) : GameRepository {
    ...
    override fun createMatch(match: Match) {
        handle.createUpdate("insert into dbo.Matches (id, gameName, state, curr_player, curr_turn, deadline_turn, created, info) " +
    ...
    override fun updateMatch(match: Match) {
        handle.createUpdate("update dbo.Matches set state = :state, curr_player = :currPlayer, " +
